...
module.exports = async (interaction) => {  let Server = await sunucuVeri.findOne({ guildID: client.guilds.cache.get(Settings.guildID) })
        ...
        if (interaction.customId === 'serverSicilUser') {
                Database.find({victimID: member.id}, async (err, res) => {
                        if (res.length <= 0) return interaction.reply({content: `${client.emojis.cache.find(x => x.name === "winclub_carpi") || "Emoji Bulunamadı"} Databasede kayıtlı bir ceza-i işlem geçmişi bulunamadı.`, ephemeral: true})
                      ...
                      if (res.length > 20) return interaction.reply({content: `${client.emojis.cache.find(x => x.name === "winclub_carpi") || "Emoji Bulunamadı"} Databasede 20'den fazla ceza-i işlemin bulundugu için cezalarını listeyemiyorum.`, ephemeral: true})                

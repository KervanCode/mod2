    ...
    execute: async (client, message, args, ClientEmbed, FlatEmbed) => {
        if (Settings["Bot.Owner"].some(member => message.author.id === member)) {
          ...
          const rowcuk = new MessageActionRow().addComponents(
            ...
            .setStyle('SUCCESS')
            .setEmoji("1033451252849328221"),
            new MessageButton()
            .setCustomId('cekilisKatilimcisi')
            .setLabel(`Çekiliş Katılımcısı`)
            .setStyle('DANGER')
            .setEmoji("1033451267114151976"),)
            ...
            message.reply(`Etkinlik,Çekiliş Katılımcısı Rol Alma Menüsü <#${Server.rolAlmaChannel}> kanalına gönderildi.`), message.react(`${client.emojis.cache.find(x => x.name === "winclub_tik")}`)

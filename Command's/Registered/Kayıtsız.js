...
module.exports = { name: "kayıtsız", aliases: ["unregister", "unreg"],  category: "Register", desc: "Kullanıcıyı Kayıtsıza Atma Komutu",
    ...
    if(!message.member.permissions.has("8") && !Server.regisyterStaff.some(rol => message.member.roles.cache.has(rol))) return message.reply(AutoReply.YetersizYetki).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "winclub_carpi")}`)
    if(!message.channel.id.includes(Server.registerChannel)) return message.reply("Bu komutu yanlızca register kanalında kullanabilirsin.").then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "winclub_carpi")}`)
    const Member = message.mentions.members.first() || message.guild.members.cache.get(args[0])
    if(!Member) return message.reply(AutoReply.UyeBelirt).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "winclub_carpi")}`)
    ...
    if (Member.roles.highest.position >= message.member.roles.highest.position && !Settings["Bot.Owner"].includes(message.author.id)) return message.reply(AutoReply.YetersizYetki).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "winclub_carpi")}`)
    ...
    Database.findOne({guildID: message.guild.id, victimID: Member.id}, (err, res) => {
        ...
        await Database.findOneAndUpdate({ guildID: message.guild.id, execID: message.author.id }, { $inc: { erkek: 0, kari: 0 } }, { upsert: true });
    ...
    message.reply({ embeds: [Embed.setDescription(`${Member} adlı kullanıcı **Kayıtsız**'a atıldı.`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "winclub_tik")}`)
    }
}
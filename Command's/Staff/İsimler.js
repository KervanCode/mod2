...
execute: async (client, message, args, ClientEmbed, FlatEmbed) => {
    ...
    if(!victim) return message.reply(AutoReply.UyeBelirt).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "winclub_carpi")}`) 
...
return message.reply("Kullanıcının isim geçmişi olmadığı için veri görüntüleyemedim.")
        ...
        ${client.emojis.cache.find(x => x.name === "winclub_carpi") || "Emoji Bulunamadı"} üyenin \`${History.length}\` adet geçmiş ismi görüntülendi.`)]}), message.react(`${client.emojis.cache.find(x => x.name === "winclub_tik")}`) 

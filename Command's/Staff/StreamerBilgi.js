...
module.exports = { name: "numara", aliases: ["stbilgi", "streamerbilgi"],  category: "Staff", desc: "StreamerNO ile yayın detaylayını görme komutus",
    ...
    let sayi = args[0]
    if(!Number(sayi) && !sayi) return message.reply(AutoReply.SayıBelirt), message.react(`${client.emojis.cache.find(x => x.name === "winclub_carpi")}`)
    Database.findOne({guildID: message.guild.id, StNumara: sayi }, async (err, yayıncı) => {
      if(yayıncı) {
       ...
       const ses = yayıncı.sestekiler.map(x => `<@${x}> - \`${x}\``).join("\n")
        ...
        ${client.emojis.cache.find(x => x.name === "winclub_soru") || "Emoji Bulunamadı"} **${args[0]}** ceza numarasına ait yayın istatistikleri;\`\`\`js
...
${ses}
`)]}), message.react(`${client.emojis.cache.find(x => x.name === "winclub_tik")}`)
            } else {
                message.reply(AutoReply.CezaNoYok).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "winclub_carpi")}`)
            }
          })
          }
        }

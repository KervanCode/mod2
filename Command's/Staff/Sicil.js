    ...
    const row = new MessageActionRow().addComponents(
      ...
      let total = Number(mute) + Number(vmute) + Number(jail) +  Number(ban)
       ...
       Database.find({victimID: member.id}, async (err, res) => {
        if (res.length <= 0) return  message.reply("Bu Kişinin herhangi bir sicil geçmişine rastlanmadı"), message.react(`${client.emojis.cache.find(x => x.name === "winclub_carpi")}`) 

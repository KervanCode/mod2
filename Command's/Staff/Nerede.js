...
module.exports = { name: "nerde", aliases: ["sesbilgi", "nerede"],  category: "Staff", desc: "Kullanıcının Hangi Ses Kanalında Oldugunu Görme Komutu",
    ...
    ${client.emojis.cache.find(x => x.name === "winclub_member") || "Emoji Bulunamadı"} Kullanıcı: ${member}, Ses kanalı ${member.voice.channel}
    ───────────────
    ${client.emojis.cache.find(x => x.name === "winclub_sescik") || "Emoji Bulunamadı"}  ${txt}
    ───────────────
    ${client.emojis.cache.find(x => x.name === "winclub_ayar") || "Emoji Bulunamadı"}  Kullanıcının Mikrafonu **${mic}** , Kulaklığı **${hop}**`)]}), message.react(`${client.emojis.cache.find(x => x.name === "winclub_tik")}`) 

...
module.exports = { name: "sil", aliases: ["temizle"],  category: "Co-owner", desc: "Kanaldaki Mesajları Silme Komutu",
    ...
    if (!member) {
        if (!args[0] || (args[0] && isNaN(args[0])) || Number(args[0]) < 1 || Number(args[0]) > 100) return message.react(client.emojis.cache.find(res => res.name === "winclub_carpi")).catch(() => { })
        ...
        if (!args[1] || (args[1] && isNaN(args[1])) || Number(args[1]) < 1 || Number(args[1]) > 100) return message.react(client.emojis.cache.find(res => res.name === "winclub_carpi")).catch(() => { })
